...
# ————————————————————————————————————————————
# Static physical positions (id → (x%, y%))
STATIC_POSITIONS = {
...
# For non‑NeoGeo: bottom-first priority
NON_NEO_PANEL_IDS = {
    '2-Button': ['1', '2'],
    '4-Button': ['1', '2', '3', '4'],
...
# For NeoGeo: its own logic (top row = letters A–D, bottom row placeholders)
NEO_PANEL_IDS = {
    '2-Button': ['1', '2'],
    '4-Button': ['3', '4', '1', '2'],
...
# Letter → ini index for NeoGeo colors/functions
NEO_BUTTON_LETTER_ORDER = {'A': 1, 'B': 2, 'C': 3, 'D': 4}

# ————————————————————————————————————————————
# RetroBat physical order & controller map (non‑Neo)
...
# Start & Coin constants
START_POS = (85, 90)
COIN_POS  = (95, 90)
JOYSTICK_DEFAULT_COLOR = 'Gray'

# List of NeoGeo ROMs (lowercase)
NEOGEO_ROMS = {
...
def generate_xml_for_rom(rom, color_cfg, func_cfg):
    ...
    for layout_type, ids in panel_ids_map.items():
        ...
        for idx, phys_id in enumerate(ids, start=1):
            ...
            # physical index from RetroBat order
            ...
            if is_neo:
                ...
                # non‑NeoGeo logic
                color_btn = get_value(color_cfg, rom,
                                      f'P1_BUTTON{idx}', 'Gray')
                func_btn  = get_value(func_cfg, rom,
                                      f'P1_BUTTON{idx}', 'None')
                btn_id    = phys_id
                controller= RB_CONTROLLER_MAP.get(phys_id, phys_id)
                game_btn  = phys_id
        ...
        # Start & Coin (same for all)
        ET.SubElement(lay, 'button',
                      id='START',
                      physical=str(len(ids) + 1),
                      controller='START',
                      gameButton='START',
                      x=str(START_POS[0]), y=str(START_POS[1]),
                      color=get_value(color_cfg, col_sec,
                                      'P1_START', 'White'),
                      function='Start')
        ET.SubElement(lay, 'button',
                      id='COIN',
                      physical=str(len(ids) + 2),
                      controller='SELECT',
                      gameButton='COIN',
...
def generate_all_xmls():
    ...
    for rom in all_roms:
        ...
        path     = os.path.join(OUTPUT_DIR, f'{rom}.xml')
        with open(path, 'wb') as f:
            f.write(pretty)

if __name__ == '__main__':
    generate_all_xmls()
    print("Génération terminée. XMLs dans", OUTPUT_DIR)
